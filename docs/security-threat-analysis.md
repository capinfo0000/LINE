...
### 1-1. 【重大】最重要: `.env` / Stripe シークレットキー漏洩
...
### 1-2. 【重大】見落としやすい第2のPII貯蔵庫: `logs/mail.log`
...
### 1-3. 【高】SQLite DB（`data/app.sqlite`）漏洩
...
### 1-4. 【重大】docroot 誤設定（設定ミス由来で最も起きやすい）
...
### 2-1. 【重大】CSV 数式インジェクション（認証不要・受動的）
...
### 2-2. 【重大】テナント間 IDOR（破れたオブジェクトレベル認可）— 確証あり
...
### 2-3. 【高】未認証エンドポイントの濫用（情報の外部“発信”悪用）
...
### 3-1. 【重大】共有 Stripe キー × オープン多テナント＝PII・金銭の合流
...
- **5-1 【中】ログインのタイミングオラクル**: `login_tenant`（`tenant.php:139`）はアカウント無→即 return、有→bcrypt 実行。応答時間差でメール存在を列挙可能。
- **5-2 【高】forgot/signup にレート制限なし**（login にはある）。
- **5-3 【中】Cookie `Secure` は条件付き**＋セッションのアイドル/絶対タイムアウトなし。
- **5-4 【中】セキュリティヘッダ皆無**（CSP / X-Frame-Options 等）→ クリックジャッキング、XSS 発生時の被害拡大。
- **5-5 【中】`tenant_id` の二重利用**: `tn_…` を公開URL（`o.php?t=`）の識別子と内部参照の両方に使用。共有リンク経由で識別子が事実上公開。
