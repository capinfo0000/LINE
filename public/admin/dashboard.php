...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string) ($_POST['action'] ?? '') === 'waiver_mode') {
    ...
    header('Location: dashboard?msg=' . rawurlencode('紹介判定モードを ' . $mode . '案 に変更しました。') . '&type=ok');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string) ($_POST['action'] ?? '') === 'signup_mode') {
    ...
    header('Location: dashboard?msg=' . rawurlencode('登録運用モードを「' . $label . '」に変更しました。') . '&type=ok');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string) ($_POST['action'] ?? '') === 'intro_gate') {
    ...
    $note = $on
        ...
        : '自己紹介ロックをOFFにしました。' . ($exempted > 0
            ...
            : '（新たに記録した会員はありません）');
    header('Location: dashboard?msg=' . rawurlencode($note) . '&type=ok');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array((string) ($_POST['action'] ?? ''), ['seed_samples', 'delete_samples'], true)) {
    ...
    if ((int) ($tenant['is_admin'] ?? 0) !== 1) {
        header('Location: dashboard?msg=' . rawurlencode('権限がありません。') . '&type=ng');
        exit;
    }
    ...
    header('Location: dashboard?msg=' . rawurlencode($m) . '&type=ok');
    exit;
}
...
<div class="card">
    ...
    <?php if ($__freeRatio >= 40): ?>
        <p class="flash flash--ng" style="margin:.4rem 0;">無料比率が高くなっています。収益を守るには「B案（課金中のみカウント）」への切替を検討してください。<strong>ただし「永久無料」を達成済みの会員は、切り替えても無料のままです。</strong></p>
    <?php endif; ?>
    <?php endif; ?>
    ...
    <div style="border-top:1px solid var(--border);margin-top:.6rem;padding-top:.6rem;">
        ...
        <p class="muted" style="font-size:.78rem;margin:6px 0 0;">A案＝無料化した紹介先も<?= (int) referral_waiver_min() ?>名に数える（無料が連鎖して広がる）／B案＝実際に課金している紹介先だけ数える。次回のcron判定から反映されます。<br>
            通常の無料は<strong>人数を割ると翌月から通常額に戻ります</strong>。紹介した<?= (int) referral_waiver_min() ?>名がそれぞれ<?= (int) referral_waiver_min() ?>名ずつ紹介すると<strong>「永久無料」</strong>になり、以後は人数が減っても外れません（<?= (int) waiver_permanent_count() ?>名）。</p>
    </div>
</div>
